    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            av_reg = sudo_reg = re.compile(pattern)
        else:
            av_ = "\\" + CMD_HANDLER
            sudo_ = "\\" + SUDO_HANDLER
            av_reg = re.compile(av_ + pattern)
        ...
        if not disable_edited:
            bot.add_event_handler(
                func, events.MessageEdited(
                    **args, outgoing=True, pattern=av_reg))
        bot.add_event_handler(
            func, events.NewMessage(**args, outgoing=True, pattern=av_reg)
        )
        if allow_sudo:
            if not disable_edited:
                bot.add_event_handler(
